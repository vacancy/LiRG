...
class Model(ModelBase):
    ...
    def train(self, in_data, nr_rows, nr_cols, nr_types):
        ...
        self.mc = numpy.ones((nr_rows, nr_types), dtype=numpy.float32)
        for i in range(nr_rows):
            for j in range(1, nr_cols):
                self.mc[i][_d(in_data[i, j], in_data[i^1, j-1])] += 1
    ...
    def predict(self, out_data, nr_rows, nr_cols, nr_types, gt_data):
        for i in range(0, nr_rows//2, 1):
            ...
            for j in range(nr_cols):
                out_data[u1, j] = (utils.argmax(self.mc[u1]) + last2)%3
                out_data[u2, j] = (utils.argmax(self.mc[u2]) + last1)%3
                self.mc[u1][_d(gt_data[u1, j], last2)] += 1
                self.mc[u2][_d(gt_data[u2, j], last1)] += 1
                last1 = gt_data[u1, j]
                last2 = gt_data[u2, j]
